...
- **Visual Type**: Table
- **Columns**:
  1. `dw.fact_transaction[transaction_id]`
...
- **Date Range**: Date picker using `dw.dim_date[full_date]` (from/to)
- **Region**: Multi-select dropdown using `dw.dim_market[region]`
- **Product Category**: Multi-select dropdown using `dw.dim_product[category]`
- **Fiscal Year**: Single select using `dw.dim_date[fiscal_year]`
...
- Blockchain Verification Status (Calculated)
- `dw.dim_farmer[farmer_id]`
...
4. **Calculated Columns**: Minimize use; prefer DAX measures for better performance.
5. **Data Types**: Optimize (INTEGER for keys, DECIMAL for measures).
